class Minibank:
        ...
        if registeruserExit==True:
            ...
            try:
                ...
                if r_password == r_password2:
                    ...
                    self.file_update(r_user_name, r_password, r_amount)
                else:
    ...
    def file_info_recall(self):
        with open("userInfo.txt",'r') as file:
            if file:
                id=1
                contents=file.readlines()
                for i in contents: 
                    username, password, amount= i.split()
                    password = int(password)
                    amount = int(amount)
                    userinfoForm2 : dict ={id:{"register_name" : username, "registerpassword" : password, "r_amount" : amount}}
                    Minibank.userInfo.update(userinfoForm2)
                    id += 1
            else:
    ...
    def file_update(self, username, password, amount):
        
        with open("userInfo.txt",'a') as file:
            if file:
                contents=f"{username} {password} {amount} \n"
                file.write(contents)
            else:
                print("something went wrong")
